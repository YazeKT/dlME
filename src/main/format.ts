...
export function buildFormatArguments(options: DownloadOptions): string[] {
  if (options.kind === 'audio') {
    ...
    if (options.audioContainer !== 'best') args.push('--audio-quality', options.audioQuality === 'best' ? '0' : `${options.audioQuality}K`)
    return args
  }
  ...
  const exact = options.exactFormatId && options.exactFormatKind === 'video' && options.sourceHasAudio !== false ? `${options.exactFormatId}+bestaudio` : options.exactFormatId
  const args = ['--format', exact || selector]
  if (options.videoContainer !== 'auto') {
    args.push('--merge-output-format', options.videoContainer === 'webm' ? 'mkv' : options.videoContainer)
